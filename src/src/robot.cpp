...
Robot::Robot(std::string &kinematic_file,
             std::vector<std::string> first_last_link_name,
             Eigen::Vector3d grav, Eigen::Matrix4d Init_mat)
    : m_JPos_lim(2), m_JTorque_lim(2) {
  ...
  //   m_robot_tree.getChain("world_link", "ee_tip_link", m_robot_chain); //wrt
  //   to ee visual
  m_robot_tree.getChain(first_last_link_name[0], first_last_link_name[1],
                        m_robot_chain);
...
void Robot::setLims(std::vector<Eigen::VectorXd> JPos_lim,
                    std::vector<Eigen::VectorXd> JTorque_lim) {
  for (int i = 0; i < 2; i++) {
...
Eigen::Vector3d Robot::getOrientRPY(Eigen::MatrixXd R) {
  ...
  orient(1) =
      std::atan2(-R(2, 0), std::sqrt(pow(R(2, 1), 2) + pow(R(2, 2), 2)));
...
std::vector<Eigen::VectorXd> Robot::getWorkspace(
    int n_samples, std::vector<std::vector<double>> space_bounds,
    double space_res) {
  //   int n_samples = 51;
  ...
  std::vector<Eigen::VectorXd> allVecs(
      m_n_joints);  // set of all vectors to combine.
  ...
  //   int n[3] = {(int) ((space_max[0]-space_bounds_min[0])/d_space), (int)
  //   ((space_max[1]-space_bounds_min[1])/d_space), (int)
  //   ((space_max[2]-space_bounds_min[2])/d_space)} ;
  ...
  std::cout << "Space matrix initialized \t" << spacematrix[0][0][0]
            << std::endl;
...
void Robot::WS(const std::vector<Eigen::VectorXd> &allVecs, int vecIndex,
               Eigen::VectorXd qSoFar, std::vector<Eigen::VectorXd> &res,
               boost::multi_array<bool, 3> &spacematrix,
               std::vector<double> space_bounds_min, double space_res) {
  if (vecIndex >= allVecs.size()) {
    ...
    // std::cout << "res_fwdin: " << res_fwdin << std::endl;
  ...
  for (int i = 0; i < allVecs[vecIndex].size(); i++) {
    ...
    WS(allVecs, vecIndex + 1, qSoFar, res, spacematrix, space_bounds_min,
       space_res);
  }
  return;
}
...
std::vector<double> Robot::DextMeasure(Eigen::VectorXd q) {
  ...
  for (int i = 0; i < q.rows(); i++) {
    ...
    v = 1 - std::exp(-4. * 10. * (m_JPos_lim[1](i) - q(i)) *
                     (-m_JPos_lim[0](i) + q(i)) /
                     pow((m_JPos_lim[1](i) - m_JPos_lim[0](i)), 2));
...
std::vector<double> Robot::DynamicDextMeasure(Eigen::VectorXd q) {
  ...
  Eigen::MatrixXd L =
      m_JTorque_lim[1].asDiagonal();  // scaling matrix of torques
  Eigen::MatrixXd J;
  getJacobian(J, q);
